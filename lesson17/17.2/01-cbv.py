...
b2 = Book(id=2, name="Le Come de Monte-Cristo", year=1844)
...
class BookView(Resource):
    def get(self, uid):  # получение данных
        try:
    ...
    def patch(self, uid):  # частичное обновление
        book = db.session.query(Book).get(uid)
        req_json = request.json
...
class AuthorsView(Resource):
    ...
    def post(self):
        ...
        return "добавлен новый автор", 201
...
class AuthorView(Resource):
    def get(self, uid):  # получение данных
        try:
            author = db.session.query(Author).filter(Author.id == uid).one()
